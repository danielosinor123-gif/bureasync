...
app.MapPost("/api/auth/test-json",async(HttpContext ctx)=>{ctx.Request.EnableBuffering();using var ms=new MemoryStream();await ctx.Request.Body.CopyToAsync(ms);var body=System.Text.Encoding.UTF8.GetString(ms.ToArray());var r=System.Text.Json.JsonSerializer.Deserialize<TestJsonRequest>(body,new System.Text.Json.JsonSerializerOptions{PropertyNameCaseInsensitive=true});if(r is null)return Results.BadRequest(new{error="Invalid JSON body",rawBody=body,bodyLength=body.Length});return Results.Ok(new{received=r.Value,rawBody=body});}).AllowAnonymous();
