...
// LLM endpoints
api.MapGet("/llm/mapping-review",async(BureauSyncDb db)=>Results.Ok(await db.MappingReviewQueues.Where(x=>x.Status=="Pending").OrderBy(x=>x.CreatedAt).Select(x=>new{x.Id,x.LenderId,x.SourceColumnName,x.SuggestedCanonicalField,x.Confidence,x.SampleValuesJson,x.Status,x.CreatedAt}).ToListAsync()));
api.MapPost("/llm/mapping-review/{id:guid}/approve",async(Guid id,BureauSyncDb db,ClaimsPrincipal p)=>{if(!(p.IsInRole(Roles.Admin)||p.IsInRole(Roles.Operator)))return Results.Forbid();var q=await db.MappingReviewQueues.FindAsync(id);if(q is null)return Results.NotFound();q.Status="Approved";var m=new LenderColumnMapping{LenderId=q.LenderId,SourceColumnName=q.SourceColumnName,CanonicalField=q.SuggestedCanonicalField,Confidence=q.Confidence,ApprovedByOperator=true};db.LenderColumnMappings.Add(m);Audit(db,p,"Mapping.Approved","MappingReviewQueue",id.ToString(),q.SuggestedCanonicalField);await db.SaveChangesAsync();return Results.Ok(new{id,status="Approved"});});
api.MapPost("/llm/mapping-review/{id:guid}/reject",async(Guid id,BureauSyncDb db,ClaimsPrincipal p)=>{if(!(p.IsInRole(Roles.Admin)||p.IsInRole(Roles.Operator)))return Results.Forbid();var q=await db.MappingReviewQueues.FindAsync(id);if(q is null)return Results.NotFound();q.Status="Rejected";Audit(db,p,"Mapping.Rejected","MappingReviewQueue",id.ToString(),q.SuggestedCanonicalField);await db.SaveChangesAsync();return Results.Ok(new{id,status="Rejected"});});
api.MapPost("/llm/explanations/{recordId:guid}",(Guid recordId,OperatorExplanationService svc)=>Results.Accepted($"/api/llm/explanations/{recordId}",new{queued=true,job="OperatorExplanation"}));
api.MapPost("/llm/root-cause/{recordId:guid}",(Guid recordId,RootCauseClassificationService svc)=>Results.Accepted($"/api/llm/root-cause/{recordId}",new{queued=true,job="RootCauseClassification"}));
api.MapPost("/llm/correction/{recordId:guid}",(Guid recordId,CorrectionDraftService svc,string bureauFormat="Default")=>Results.Accepted($"/api/llm/correction/{recordId}",new{queued=true,job="CorrectionDraft",bureauFormat}));
