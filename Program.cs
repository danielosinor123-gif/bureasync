...
static string FixJson(string json){if(string.IsNullOrWhiteSpace(json))return json;var result=new System.Text.StringBuilder(json.Length+100);bool inString=false;bool escape=false;for(int i=0;i<json.Length;i++){char c=json[i];if(escape){result.Append(c);escape=false;continue;}if(c=='\\'){result.Append(c);escape=true;continue;}if(c=='"'){inString=!inString;result.Append(c);continue;}if(!inString){if(c==':'||c==','||c=='{'||c=='['||c=='}'||c==']'){result.Append(c);continue;}if(char.IsLetterOrDigit(c)||c=='_'||c=='$'||c=='@'||c=='.'){int start=i;while(i<json.Length&&(char.IsLetterOrDigit(json[i])||json[i]=='_'||json[i]=='$'||json[i]=='@'||json[i]=='.'))i++;string token=json.Substring(start,i-start);i--;if(i+1<json.Length&&json[i+1]==':'){result.Append('"');result.Append(token);result.Append('"');}else if(token=="true"||token=="false"||token=="null"){result.Append(token);}else{result.Append('"');result.Append(token);result.Append('"');}continue;}}result.Append(c);}return result.ToString();}
app.MapPost("/api/auth/register",async(HttpContext ctx,BureauSyncDb db)=>{ctx.Request.EnableBuffering();using var ms=new MemoryStream();await ctx.Request.Body.CopyToAsync(ms);var body=System.Text.Encoding.UTF8.GetString(ms.ToArray());var fixedJson=FixJson(body);var r=System.Text.Json.JsonSerializer.Deserialize<RegisterRequest>(fixedJson,new System.Text.Json.JsonSerializerOptions{PropertyNameCaseInsensitive=true});if(r is null)return Results.BadRequest(new{error="Invalid JSON body"});if(await db.Users.AnyAsync())return Results.StatusCode(StatusCodes.Status403Forbidden);if(r.Password.Length<14)return Results.BadRequest(new{error="Password must be at least 14 characters."});if(r.Role!=Roles.Admin)return Results.BadRequest(new{error="The one-time bootstrap account must be a BureauAdmin."});var email=r.Email.Trim().ToLowerInvariant();db.Users.Add(new User{Email=email,PasswordHash=HashPassword(r.Password),Role=Roles.Admin});await db.SaveChangesAsync();return Results.Created("/api/auth/login",new{message="Initial BureauAdmin account created. Use the authenticated user endpoint to provision additional users."});}).AllowAnonymous();
app.MapPost("/api/auth/login",async(HttpContext ctx,BureauSyncDb db)=>{ctx.Request.EnableBuffering();using var ms=new MemoryStream();await ctx.Request.Body.CopyToAsync(ms);var body=System.Text.Encoding.UTF8.GetString(ms.ToArray());var fixedJson=FixJson(body);var r=System.Text.Json.JsonSerializer.Deserialize<LoginRequest>(fixedJson,new System.Text.Json.JsonSerializerOptions{PropertyNameCaseInsensitive=true});if(r is null)return Results.BadRequest(new{error="Invalid JSON body"});var u=await db.Users.SingleOrDefaultAsync(x=>x.Email==r.Email.Trim().ToLowerInvariant());if(u is null||!u.IsActive||!VerifyPassword(r.Password,u.PasswordHash))return Results.Unauthorized();return Results.Ok(IssueToken(u,builder.Configuration,key));}).AllowAnonymous();
...
// LLM endpoints with API key auth
var llmApi=app.MapGroup("/api/llm");
llmApi.MapGet("/mapping-review",async(BureauSyncDb db)=>Results.Ok(await db.MappingReviewQueues.Where(x=>x.Status=="Pending").OrderBy(x=>x.CreatedAt).Select(x=>new{x.Id,x.LenderId,x.SourceColumnName,x.SuggestedCanonicalField,x.Confidence,x.SampleValuesJson,x.Status,x.CreatedAt}).ToListAsync()));
llmApi.MapPost("/mapping-review/{id:guid}/approve",async(Guid id,BureauSyncDb db)=>{var q=await db.MappingReviewQueues.FindAsync(id);if(q is null)return Results.NotFound();q.Status="Approved";var m=new LenderColumnMapping{LenderId=q.LenderId,SourceColumnName=q.SourceColumnName,CanonicalField=q.SuggestedCanonicalField,Confidence=q.Confidence,ApprovedByOperator=true};db.LenderColumnMappings.Add(m);await db.SaveChangesAsync();return Results.Ok(new{id,status="Approved"});});
llmApi.MapPost("/mapping-review/{id:guid}/reject",async(Guid id,BureauSyncDb db)=>{var q=await db.MappingReviewQueues.FindAsync(id);if(q is null)return Results.NotFound();q.Status="Rejected";await db.SaveChangesAsync();return Results.Ok(new{id,status="Rejected"});});
llmApi.MapPost("/explanations/{recordId:guid}",(Guid recordId,OperatorExplanationService svc)=>Results.Accepted($"/api/llm/explanations/{recordId}",new{queued=true,job="OperatorExplanation"}));
llmApi.MapPost("/root-cause/{recordId:guid}",(Guid recordId,RootCauseClassificationService svc)=>Results.Accepted($"/api/llm/root-cause/{recordId}",new{queued=true,job="RootCauseClassification"}));
llmApi.MapPost("/correction/{recordId:guid}",(Guid recordId,CorrectionDraftService svc,string bureauFormat="Default")=>Results.Accepted($"/api/llm/correction/{recordId}",new{queued=true,job="CorrectionDraft",bureauFormat}));
