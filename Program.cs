...
app.MapPost("/api/auth/test-json",async(HttpContext ctx)=>{ctx.Request.EnableBuffering();var body=await new StreamReader(ctx.Request.Body).ReadToEndAsync();ctx.Request.Body.Position=0;var r=await ctx.Request.ReadFromJsonAsync<TestJsonRequest>();if(r is null)return Results.BadRequest(new{error="Invalid JSON body",rawBody=body});return Results.Ok(new{received=r.Value,rawBody=body});}).AllowAnonymous();
