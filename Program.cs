...
app.MapPost("/api/auth/test-json",async(HttpContext ctx)=>{ctx.Request.EnableBuffering();using var ms=new MemoryStream();await ctx.Request.Body.CopyToAsync(ms);var bytes=ms.ToArray();var body=System.Text.Encoding.UTF8.GetString(bytes);var hex=BitConverter.ToString(bytes);try{var r=System.Text.Json.JsonSerializer.Deserialize<TestJsonRequest>(body,new System.Text.Json.JsonSerializerOptions{PropertyNameCaseInsensitive=true});if(r is null)return Results.BadRequest(new{error="Deserialization returned null",rawBody=body,bodyLength=body.Length,hex=hex,firstBytes=bytes.Take(20).ToArray()});return Results.Ok(new{received=r.Value,rawBody=body,hex=hex});}catch(Exception ex){return Results.BadRequest(new{error="Deserialization failed",exception=ex.Message,rawBody=body,bodyLength=body.Length,hex=hex,firstBytes=bytes.Take(20).ToArray()});}}).AllowAnonymous();
app.MapPost("/api/auth/register",async(HttpContext ctx,BureauSyncDb db)=>{var r=await ctx.Request.ReadFromJsonAsync<RegisterRequest>();if(r is null)return Results.BadRequest(new{error="Invalid JSON body"});if(await db.Users.AnyAsync())return Results.StatusCode(StatusCodes.Status403Forbidden);if(r.Password.Length<14)return Results.BadRequest(new{error="Password must be at least 14 characters."});if(r.Role!=Roles.Admin)return Results.BadRequest(new{error="The one-time bootstrap account must be a BureauAdmin."});var email=r.Email.Trim().ToLowerInvariant();db.Users.Add(new User{Email=email,PasswordHash=HashPassword(r.Password),Role=Roles.Admin});await db.SaveChangesAsync();return Results.Created("/api/auth/login",new{message="Initial BureauAdmin account created. Use the authenticated user endpoint to provision additional users."});}).AllowAnonymous();
app.MapPost("/api/auth/login",async(HttpContext ctx,BureauSyncDb db)=>{var r=await ctx.Request.ReadFromJsonAsync<LoginRequest>();if(r is null)return Results.BadRequest(new{error="Invalid JSON body"});var u=await db.Users.SingleOrDefaultAsync(x=>x.Email==r.Email.Trim().ToLowerInvariant());if(u is null||!u.IsActive||!VerifyPassword(r.Password,u.PasswordHash))return Results.Unauthorized();return Results.Ok(IssueToken(u,builder.Configuration,key));}).AllowAnonymous();
