...
app.MapPost("/api/auth/test-json",async(HttpContext ctx)=>{ctx.Request.EnableBuffering();using var ms=new MemoryStream();await ctx.Request.Body.CopyToAsync(ms);var body=System.Text.Encoding.UTF8.GetString(ms.ToArray());ms.Position=0;ctx.Request.Body.Position=0;var r=await ctx.Request.ReadFromJsonAsync<TestJsonRequest>();if(r is null)return Results.BadRequest(new{error="Invalid JSON body",rawBody=body,bodyLength=body.Length});return Results.Ok(new{received=r.Value,rawBody=body});}).AllowAnonymous();
