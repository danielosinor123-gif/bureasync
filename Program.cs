...
app.MapPost("/api/auth/test-json",async(HttpContext ctx)=>{var body=await new StreamReader(ctx.Request.Body).ReadToEndAsync();var r=System.Text.Json.JsonSerializer.Deserialize<TestJsonRequest>(body,new System.Text.Json.JsonSerializerOptions{PropertyNameCaseInsensitive=true});if(r is null)return Results.BadRequest(new{error="Invalid JSON",body});return Results.Ok(new{received=r.value,rawBody=body});}).AllowAnonymous();
