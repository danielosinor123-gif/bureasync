...
static string FixJson(string json){if(string.IsNullOrWhiteSpace(json))return json;var result=new System.Text.StringBuilder(json.Length+50);bool inString=false;bool escape=false;for(int i=0;i<json.Length;i++){char c=json[i];if(escape){result.Append(c);escape=false;continue;}if(c=='\\'){result.Append(c);escape=true;continue;}if(c=='"'){inString=!inString;result.Append(c);continue;}if(!inString&&c==':'){result.Append(c);continue;}if(!inString&&(c=='{'||c==',')){result.Append(c);int j=i+1;while(j<json.Length&&char.IsWhiteSpace(json[j])){result.Append(json[j]);j++;}int start=j;while(j<json.Length&&(char.IsLetterOrDigit(json[j])||json[j]=='_'||json[j]=='$'))j++;if(j>start){result.Append('"');result.Append(json,start,j-start);result.Append('"');i=j-1;continue;}}result.Append(c);}return result.ToString();}
