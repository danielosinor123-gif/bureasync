...
app.MapPost("/api/auth/test-json",async(HttpContext ctx)=>{var r=await ctx.Request.ReadFromJsonAsync<TestJsonRequest>();if(r is null)return Results.BadRequest(new{error="Invalid JSON body"});return Results.Ok(new{received=r.value});}).AllowAnonymous();
